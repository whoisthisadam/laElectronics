...
//                        .reduce(0L, Long::sum)
//        );
//        order.setSubscriptions(orderCreateDto.getProducts().stream().map(x -> subscriptionRepository.findById(x).orElseThrow(EntityNotFoundException::new)).collect(Collectors.toSet()));
