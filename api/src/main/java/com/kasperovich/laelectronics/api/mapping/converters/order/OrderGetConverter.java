...
//        List<Subscription> subscriptions =order.getSubscriptions().stream().toList();
//        List<ProductGetDto> productDtoList=productListMapper.toDto(subscriptions);
//        productDtoList.forEach(x->{
//            Optional<Category> category=Optional.ofNullable(subscriptions.get(productDtoList.indexOf(x)).getCategory());
